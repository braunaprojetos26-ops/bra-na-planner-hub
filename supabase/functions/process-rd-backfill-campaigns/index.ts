...
async function reinvokeSelf(jobId: string, rdUserId?: string) {
  ...
  console.log(`[backfill-campaigns] Re-invoking self for job ${jobId}`);
  try {
    const res = await fetch(`${supabaseUrl}/functions/v1/process-rd-backfill-campaigns`, {
      method: "POST",
      headers: {
    ...
    console.log(`[backfill-campaigns] Re-invoke response: ${res.status}`);
  } catch (err) {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (checkpoint.phase === "fetching_deals") {
      ...
      console.log(`[backfill-campaigns] Job ${jobId}: fetching all deals`);
      ...
      console.log(`[backfill-campaigns] Found ${dealIds.length} total deals`);
    ...
    if (checkpoint.phase === "processing_deals") {
      ...
      while (dealIndex < dealIds.length) {
        ...
        try {
          // Fetch deal details to get campaign
          ...
          const campaign = deal.campaign as { _id?: string; name?: string } | null;
          ...
          for (const rdContact of rdContacts as Array<Record<string, unknown>>) {
            ...
            if (localContactId) {
              // Update campaign field on the contact
      ...
      console.log(`[backfill-campaigns] Job ${jobId} done: ${updated} updated, ${skipped} skipped, ${errors} errors`);
    }
    ...
    console.error("[backfill-campaigns] Error:", error);
    await updateJob(jobId, {
      status: "error",
