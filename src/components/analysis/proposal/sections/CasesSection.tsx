...
function isPointsCase(caseItem: PlannerCase): boolean {
  return caseItem.title.toLowerCase().includes('milhas');
}
    ...
    <section className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {cases.slice(0, 6).map((caseItem) => {
          const isPoints = isPointsCase(caseItem);
                  ...
                  {caseItem.initial_value && (
                    <div>
                      ...
                      <p className="font-medium text-foreground">
                        {isPoints ? formatPoints(caseItem.initial_value) : formatCurrency(caseItem.initial_value)}
                      </p>
                    </div>
                  )}
                  {caseItem.final_value && (
                    <div>
                      ...
                      <p className="font-medium text-green-600">
                        {isPoints ? formatPoints(caseItem.final_value) : formatCurrency(caseItem.final_value)}
                      </p>
                    </div>
                  )}
                </div>
              ...
              {caseItem.advantage && (
                <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-3 text-center">
                  ...
                  <p className="font-bold text-green-600">
                    +{isPoints ? formatPoints(caseItem.advantage) : formatCurrency(caseItem.advantage)}
                  </p>
                </div>
              )}
            </div>
